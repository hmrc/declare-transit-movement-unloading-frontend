...
class KevController @Inject() (
  ...
  def onPageLoad(arrivalId: ArrivalId, mode: Mode): Action[AnyContent] = (identify andThen getData(arrivalId) andThen requireData).async {
    implicit request =>
      val preparedForm = request.userAnswers.get(KevPage) match {
      ...
      renderer.render("kev.njk", json).map(Ok(_))
  }
      ...
      form
        ...
        .fold(
          formWithErrors => {
            ...
            renderer.render("kev.njk", json).map(BadRequest(_))
          },
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(KevPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(KevPage, mode, updatedAnswers))
        )
  }
}
